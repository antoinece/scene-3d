...
void Scene3D::Update(const float dt)
{
  ...
  const auto projection = glm::perspective(glm::radians(45.0f), (float)1280 / (float)720, 0.1f, 10000.0f);


  program_.SetMat4("projection", projection);
  program_.SetMat4("view", camera_.view());
  ...
  skybox_program_.Use();
  skybox_program_.SetMat4("view", glm::mat4(glm::mat3(camera_.view())));
...
void Scene3D::UpdateCamera(float dt)
{
  // Get keyboard state
...
void Scene3D::DrawImGui()
{
  ...
  //ImGui::SliderFloat("Model Size", &model_scale_, 1.0f, 100.0f, "%.1f");
